...
import { addNewProductAsync, clearErrors, clearSuccess } from '../../features/Product/productSlice';
        ...
        <>
            <div className="container flex md:flex-row flex-col gap-3">
                ...
                <div className="add-product-container md:w-[80%] w-full p-3">
                    ...
                    {pending && <Loader />}
                    {!pending && <form className='w-[70%] mx-auto bg-white shadow-md p-3' onSubmit={handleAddProduct}>
                        ...
                        <input type="number" name='price' placeholder='Product Price' className='w-[90%] mx-auto block border-2 py-2 px-2 my-3' value={price} onChange={(e) => setPrice(e.target.value)} required />
                        ...
                        <input type="number" placeholder='Product Stock' name='stock' className='w-[90%] mx-auto block border-2 py-2 px-2 my-3' value={stock} onChange={(e) => setStock(e.target.value)} required />
                        ...
                        <input type="file" name='avatar' className='w-[90%] mx-auto block border-2 py-2 px-2 my-3' multiple onChange={handleImagesChange} accept='image/*' required />

                        <div className="images-preview flex overflow-auto gap-2">
                            {imagesPreivew && imagesPreivew.map((image, index) => (
                                <img src={image} alt="Product Preview" key={index} className='w-1/6 object-cover object-center' />
                            ))}
                        </div>

                        <button className='w-[90%] mx-auto block text-white text-lg rounded py-2 px-2 my-5 bg-red-600 hover:bg-red-500' type='submit'>Add</button>
